<template>
    <div class="container-fluid py-4" style="background-color: #f8fafc; min-height: 100vh;">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
            <div>
                <button class="btn btn-sm text-secondary fw-medium border-0 bg-transparent px-0 mb-2 d-flex align-items-center" @click="quayLai" style="transition: all 0.2s;" onmouseover="this.style.color='#667eea'" onmouseout="this.style.color=''">
                ...
                <p class="text-muted mb-0" style="font-size: 0.95rem;">
                    <span class="badge rounded-pill px-2 py-1 me-1" style="background-color: #e0e7ff; color: #4f46e5;">ID: {{ bai_hoc_id || '—' }}</span>
                    <span v-if="tieu_de_bai_hoc" class="fw-medium text-secondary">{{ tieu_de_bai_hoc }}</span>
                </p>
            </div>
            <div>
                <button class="btn text-white shadow-sm rounded-pill px-4 py-2 fw-medium border-0" data-bs-toggle="modal" data-bs-target="#tuVungModal" @click="themTuVung" :disabled="!bai_hoc_id" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                ...
                <div class="table-responsive">
                    <table class="table align-middle mb-0 border-0">
                        <thead style="background-color: #f1f5f9;">
                            <tr>
                                <th class="ps-4 py-3 text-secondary fw-bold text-uppercase border-0 text-center" style="font-size: 0.75rem; letter-spacing: 0.5px; width: 5%;">TT</th>
                                <th class="py-3 text-secondary fw-bold text-uppercase border-0 text-center" style="font-size: 0.75rem; letter-spacing: 0.5px; width: 10%;">Hình ảnh</th>
                                <th class="py-3 text-secondary fw-bold text-uppercase border-0" style="font-size: 0.75rem; letter-spacing: 0.5px; width: 25%;">Từ chuẩn</th>
                                <th class="py-3 text-secondary fw-bold text-uppercase border-0" style="font-size: 0.75rem; letter-spacing: 0.5px; width: 20%;">Phiên âm</th>
                                <th class="py-3 text-secondary fw-bold text-uppercase border-0 text-center" style="font-size: 0.75rem; letter-spacing: 0.5px; width: 15%;">Cấp độ</th>
                                <th class="pe-4 py-3 text-secondary fw-bold text-uppercase text-end border-0 text-center" style="font-size: 0.75rem; letter-spacing: 0.5px; width: 15%;">Thao tác</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr v-else-if="danh_sach_tu_vung.length === 0">
                                <td colspan="6" class="p-4 border-0">
                                    <div class="text-center py-5 rounded-4" style="border: 2px dashed #e2e8f0; background-color: #f8fafc;">
                                        ...
                                        <p class="text-muted small mb-0">Hãy bấm "Thêm từ mới" để tạo nội dung luyện âm cho bé.</p>
                                    </div>
                                </td>
                            </tr>

                            <tr v-for="tu in danh_sach_tu_vung" :key="tu.id" style="transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#f8fafc'" onmouseout="this.style.backgroundColor='transparent'">
                                <td class="ps-4 text-secondary fw-semibold text-center" style="border-bottom: 1px solid #f1f5f9;">{{ tu.thu_tu }}</td>
                                ...
                                <td style="border-bottom: 1px solid #f1f5f9;">
                                    <div class="d-flex align-items-center">
                                        <button type="button" class="btn btn-sm rounded-circle d-flex align-items-center justify-content-center me-3 border-0"
                                            ...
                                            :disabled="!tu.am_thanh_mau_url"
                                            @click.prevent="phatAmMau(tu)">
                                ...
                                <td style="border-bottom: 1px solid #f1f5f9;">
                                    <span class="text-secondary d-inline-block text-truncate fw-medium" style="max-width: 250px;" :title="tu.phien_am">
                                        {{ tu.phien_am || '—' }}
                                    </span>
                                </td>
                                ...
                                <td class="pe-4 text-center" style="border-bottom: 1px solid #f1f5f9;">
                                    <div class="d-flex justify-content-center gap-1">
                                        <button type="button" class="btn btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 bg-transparent text-primary" 
                                                style="width: 36px; height: 36px; transition: all 0.2s;" onmouseover="this.style.backgroundColor='#e0e7ff'" onmouseout="this.style.backgroundColor='transparent'"
                                                data-bs-toggle="modal" data-bs-target="#tuVungModal" @click="suaTuVung(tu)" title="Chỉnh sửa">
                                            <i class="fa-regular fa-pen-to-square"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm rounded-circle d-flex align-items-center justify-content-center border-0 bg-transparent text-danger" 
                                                style="width: 36px; height: 36px; transition: all 0.2s;" onmouseover="this.style.backgroundColor='#fee2e2'" onmouseout="this.style.backgroundColor='transparent'"
                                                data-bs-toggle="modal" data-bs-target="#xoaTuVungModal" @click="xoaTuVung(tu)" title="Xóa">
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content border-0 shadow rounded-4 overflow-hidden">
                    <div class="modal-header bg-light border-bottom-0 p-4">
                        <div class="d-flex align-items-center">
                            <div class="rounded-circle d-flex align-items-center justify-content-center me-3" 
                                 :class="isEdit ? 'bg-primary-subtle text-primary' : 'bg-success-subtle text-success'" style="width: 45px; height: 45px;">
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-4">
                            <div class="col-md-8">
                                <label class="form-label fw-semibold text-dark mb-2">Từ chuẩn <span class="text-danger">*</span></label>
                                <input type="text" class="form-control rounded-3 py-2 bg-light border-0 shadow-none" v-model="tuVungForm.tu_chuan" placeholder="VD: Con Lợn, Mặt Trời..." required>
                            </div>
                            
                            <div class="col-md-4">
                                <label class="form-label fw-semibold text-dark mb-2">Thứ tự hiển thị</label>
                                <input type="number" class="form-control rounded-3 py-2 bg-light border-0 shadow-none" v-model.number="tuVungForm.thu_tu" min="1" placeholder="VD: 1, 2, 3...">
                            </div>

                            <div class="col-md-6">
                                <label class="form-label fw-semibold text-dark mb-2">Phiên âm / Hướng dẫn</label>
                                <input type="text" class="form-control rounded-3 py-2 bg-light border-0 shadow-none" v-model="tuVungForm.phien_am" placeholder="VD: Âm L uốn cong lưỡi...">
                            </div>

                            <div class="col-md-6">
                                <label class="form-label fw-semibold text-dark mb-2">Cấp độ</label>
                                <select class="form-select rounded-3 py-2 bg-light border-0 shadow-none" v-model="tuVungForm.cap_do" style="cursor: pointer;">
                            ...
                            <div class="col-md-12">
                                <label class="form-label fw-semibold text-dark mb-2">Link Hình ảnh minh họa (URL)</label>
                                <div class="input-group">
                                    <span class="input-group-text bg-light border-0"><i class="fa-regular fa-image text-muted"></i></span>
                                    <input type="text" class="form-control py-2 bg-light border-0 shadow-none" v-model="tuVungForm.hinh_anh_url" placeholder="https://... hoặc đường dẫn file ảnh">
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text bg-light border-0"><i class="fa-solid fa-microphone-lines text-muted"></i></span>
                                    <input type="text" class="form-control py-2 bg-light border-0 shadow-none" v-model="tuVungForm.am_thanh_mau_url" placeholder="https://... hoặc đường dẫn file MP3/WAV">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal-footer border-top-0 p-4 pt-0">
                        <button type="button" class="btn btn-light fw-medium px-4 py-2 rounded-3 border" data-bs-dismiss="modal">Hủy bỏ</button>
                        <button type="button" class="btn text-white fw-medium px-4 py-2 rounded-3 border-0 shadow-sm" :disabled="saving" @click="luuTuVung" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
            ...
            <div class="modal-dialog modal-sm modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg rounded-4">
                    <div class="modal-body text-center p-4 p-md-5">
                        <div class="d-inline-flex align-items-center justify-content-center bg-danger-subtle rounded-circle mb-4" style="width: 80px; height: 80px;">
                            <i class="fa-solid fa-triangle-exclamation text-danger" style="font-size: 2.5rem;"></i>
                        </div>
                        <h5 class="fw-bold mb-3 text-dark">Xác nhận xóa?</h5>
                        <p class="text-muted mb-4" style="font-size: 0.95rem;">
                            Bạn có chắc chắn muốn xóa từ <br>
                            <strong class="text-danger fs-5">{{ tuVungXoa?.tu_chuan }}</strong><br>
                            khỏi bài học này không?
                        </p>
                        <div class="d-flex justify-content-center gap-2">
                            <button type="button" class="btn btn-light fw-medium px-4 border" data-bs-dismiss="modal">Hủy</button>
                            <button type="button" class="btn btn-danger fw-medium px-4 shadow-sm" :disabled="deleting" @click="xacNhanXoa">
