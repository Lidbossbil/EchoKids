<template>
  <div
    :class="
      floating ? 'position-fixed bottom-0 end-0 m-4' : 'chatbox-page-shell'
    "
    :style="floating ? 'z-index: 9999;' : ''"
  >
    <!-- CHAT LIST VIEW (Hình 1) -->
    <transition name="chat-window">
      <div
        v-if="isOpen && !selectedChat"
        :class="[
          'card shadow-lg border-0 mb-3',
          floating ? 'chat-container chat-list' : 'chat-page-window chat-list',
        ]"
        :style="{
          width: floating ? listWidth + 'px' : '100%',
          height: floating ? listHeight + 'px' : 'calc(100vh - 150px)',
          borderRadius: '1.25rem',
          overflow: 'hidden',
          backgroundColor: '#fff',
          transformOrigin: 'bottom right',
          cursor: floating && isResizingList ? 'nwse-resize' : 'default',
        }"
        @mouseup="stopResizeList"
        @mousemove="handleResizeList"
      >
        <div
          class="card-header d-flex justify-content-between align-items-center py-3 border-0 chat-list-header"
        >
        ...
        <!-- Tabs -->
        <div
          class="d-flex gap-2 px-3 py-3 border-bottom chat-tabs-wrap"
          style="overflow-x: auto"
        >
          <button
            @click="activeTab = 'all'"
            :class="{ active: activeTab === 'all' }"
            class="btn btn-sm rounded-pill chat-tab-btn"
        ...
        <div
          class="overflow-auto"
          style="height: calc(100% - 200px); scroll-behavior: smooth"
        >
          <div
          ...
          >
            <img
              v-if="chat.avatar"
              :src="chat.avatar"
              class="rounded-circle me-3 messenger-list-avatar"
            ...
            >
              <i
                v-if="chat.type === 'ai'"
                :class="[branding.logo_icon, 'brand-avatar-glyph']"
              ></i>
            ...
            <div class="flex-grow-1" style="min-width: 0">
              ...
              >
                <h6
                  class="mb-0 fw-bold messenger-row-name"
                  style="
                    max-width: 80%;
                    overflow: hidden;
                ...
                >
                  {{ chat.name }}
                  <span v-if="isTeacherMode && chat.lessonTitle" class="fw-normal text-secondary" style="font-size: 0.85em;">
                    - {{ chat.lessonTitle }}
                  </span>
                </h6>
                <small
                  v-if="chat.type === 'teacher'"
                  class="text-muted flex-shrink-0 ms-2"
                  >{{ formatTime(chat.lastMessageTime) }}</small
                >
              </div>
              <p
      ...
      <div
        ...
        :class="[
          ...
          floating
            ...
            : 'chat-page-window chat-detail',
        ]"
        :style="{
          width: floating ? chatWidth + 'px' : '100%',
          height: floating ? chatHeight + 'px' : 'calc(100vh - 150px)',
          borderRadius: '1.25rem',
          overflow: 'hidden',
          backgroundColor: '#fff',
          transformOrigin: 'bottom right',
          cursor: floating && isResizing ? 'nwse-resize' : 'default',
        }"
        @mouseup="stopResize"
        @mousemove="handleResize"
      >
        <div
          class="card-header d-flex justify-content-between align-items-center py-3 border-0 chat-detail-header"
          style="gap: 0.5rem"
        >
          <button
          ...
          >
            ...
            <div style="min-width: 0">
              ...
              <small v-if="selectedChat.type === 'ai'" class="text-muted"
                >Trợ lý học tập AI</small
              >
              <small v-else-if="isTeacherMode && selectedChat.lessonTitle" class="text-muted"
                >Bài: {{ selectedChat.lessonTitle }}</small
              >
            </div>
          </div>
        ...
        <!-- Messages Body -->
        <div
          class="card-body overflow-auto d-flex flex-column gap-3 p-3"
          ref="chatBody"
        ...
        >
          ...
          >
            ...
            >
              <div class="d-flex align-items-end gap-2">
                <img
                  v-if="selectedChat?.avatar"
                  :src="selectedChat.avatar"
                  class="message-peer-avatar"
                  alt="avatar giáo viên"
                />
                <div
                  v-else
                  class="message-peer-avatar message-peer-avatar-fallback"
                >
                  <i class="fa fa-user"></i>
                </div>
                <div
                  class="px-3 py-2 shadow-sm text-dark peer-message-bubble"
                  :class="{ 'clickable-message': !!msg.action_url }"
                  @click="
                    msg.action_url ? navigateToAction(msg.action_url) : null
                  "
                  style="
                    font-size: 0.95rem;
                    line-height: 1.4;
                ...
                >
                  ...
                  <div v-if="msg.action_url" class="mt-1">
                    <a
                      href="#"
                      class="action-link-emphasis"
                      @click.stop.prevent="navigateToAction(msg.action_url)"
                    >
                      Click vào đây
                    </a>
                  </div>
            ...
            >
              ...
              >
                ...
                <div v-if="msg.action_url" class="mt-2">
                  <button
                    @click.prevent="navigateToAction(msg.action_url)"
                    class="btn btn-sm btn-primary w-100 rounded-pill fw-bold shadow-sm"
                    style="
                      font-size: 0.85rem;
              ...
              <div class="d-flex align-items-center gap-2 mt-1">
                ...
                >
                <button
                  @click="playAiMessage(msg)"
                  class="btn btn-sm rounded-circle d-flex align-items-center justify-content-center text-primary"
                  style="
                    width: 24px;
                    height: 24px;
                    background: #f0f7ff;
                  ...
                  "
                  :title="
                    playingMessageId === msg.id ? 'Dừng đọc' : 'Nghe cô đọc'
                  "
                >
                  <i
                    :class="
            ...
            <div
              v-else
              class="d-flex flex-column align-items-end user-message-wrap"
            >
              <div
                class="px-3 py-2 shadow-sm text-white user-message-bubble"
                :class="{ 'clickable-message': !!msg.action_url }"
                @click="
                  msg.action_url ? navigateToAction(msg.action_url) : null
                "
                style="
                  ...
                  background: linear-gradient(135deg, #ff8c6b, #fe5d37);
                "
              >
                {{ msg.text }}
                <div v-if="msg.action_url" class="mt-1">
                  <a
                    href="#"
                    class="action-link-emphasis action-link-emphasis-on-primary"
                    @click.stop.prevent="navigateToAction(msg.action_url)"
                  >
                    Click vào đây
                  </a>
              ...
              <small
                v-if="selectedChat?.type === 'teacher'"
              ...
              <small
                v-if="msg.time"
                class="message-time message-time--self mt-1"
                >{{ msg.time }}</small
              >
            </div>
          </div>
          ...
          <div
            v-if="isTyping && selectedChat?.type !== 'teacher'"
            class="d-flex justify-content-start typing-indicator"
          >
            <div
              class="px-3 py-2 shadow-sm fst-italic text-muted small d-flex align-items-center gap-2"
              style="
                border-radius: 18px 18px 18px 0;
                background-color: #ffffff;
                border: 1px solid #ffe0d9;
        ...
        <div class="card-footer p-3 bg-white border-top">
          ...
          <div class="input-group align-items-center">
            ...
            <button
              ...
              :class="{
                'recording-pulse text-danger border-danger': isRecording,
              }"
          ...
          <transition name="fade">
            ...
            >
              ...
              <small class="text-danger fw-bold mt-1" style="font-size: 0.8rem">
                Con hãy nói đi, cô đang nghe nè...
              </small>
            </div>
          </transition>
        </div>
...
export default {
  ...
  computed: {
    ...
    totalTeacherUnread() {
      return this.teacherChats.reduce(
    ...
    chatList() {
      ...
      if (this.activeLessonTeacherChat) {
        const hasRealChatSameTeacher = this.teacherChats.some(
          (c) =>
            Number(c.teacherId) ===
            Number(this.activeLessonTeacherChat.teacherId),
        );
  ...
  methods: {
    ...
    renderMarkdown(text) {
      if (!text) return "";
      // Simple lightweight markdown-like parser
      let html = text
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        ...
        .replace(/(<li>.*<\/li>)/s, '<ul class="ps-3 mb-0">$1</ul>')
        .replace(/\n/g, "<br>");
      return html;
    },
    async playAiMessage(msg) {
      if (!msg?.text) return;

      // If same message is playing, stop it
      if (this.playingMessageId === msg.id && this.audioPlayer) {
    ...
    getSessionStorageKey() {
      return this.isTeacherMode
        ? "chat_session_id_teacher"
        : "chat_session_id_student";
    },
    getAuthToken() {
      if (this.isTeacherMode) {
    ...
    syncActiveLessonContext() {
      ...
      if (this.activeLessonContext?.teacher_id) {
        this.activeLessonTeacherChat = {
          ...
          avatar: null,
          lastMessage: this.activeLessonContext.lesson_title
            ? `Bài: ${this.activeLessonContext.lesson_title}`
          ...
          this.isTeacherMode
            ? {
                ...
                avatar: row.studentAvatar
                  ? this.resolveAvatarUrl(row.studentAvatar)
                  : null,
            ...
            : {
                ...
                avatar: row.teacher_avatar
                  ? this.resolveAvatarUrl(row.teacher_avatar)
                  : null,
    ...
    subscribeToTeacherSession(sessionId) {
      if (
        !window.Echo ||
        !sessionId ||
        this.subscribedSessionIds.has(sessionId)
      ) {
        return;
      }
      const channelName = `chat-session.${sessionId}`;
      const listenEvent = this.isTeacherMode
        ? ".StudentSentMessage"
        : ".TeacherSentMessage";
      window.Echo.private(channelName).listen(listenEvent, (e) => {
        ...
        const incomingTime = e?.message?.created_at
          ? new Date(e.message.created_at)
          : new Date();
        const chat = this.teacherChats.find(
          (c) => Number(c.sessionId) === Number(sessionId),
        );
        if (!chat) {
        ...
        if (
          this.selectedChat?.type === "teacher" &&
          Number(this.selectedChat.sessionId) === Number(sessionId)
        ) {
          ...
          this.teacherChats = [
            chat,
            ...this.teacherChats.filter((c) => c.id !== chat.id),
          ];
        }
      });
    ...
    selectChat(chat) {
      ...
      if (
        ...
        merged.lessonId = Number(
          this.activeLessonContext.lesson_id || merged.lessonId || 0,
        );
        merged.lessonTitle =
          this.activeLessonContext.lesson_title || merged.lessonTitle || "";
      }
      merged.unread = 0;
    ...
    async ensureAiSessionReady() {
      ...
      const cached = Number(
        localStorage.getItem(this.getSessionStorageKey()) || 0,
      );
    ...
    handleResize(e) {
      ...
      this.chatWidth = Math.max(
        minWidth,
        Math.min(this.startWidth - deltaX, maxWidth),
      );
      this.chatHeight = Math.max(
        minHeight,
        Math.min(this.startHeight - deltaY, maxHeight),
      );
    },
    stopResize() {
      if (!this.isResizing) return;
    ...
    async loadAiHistory() {
      ...
      try {
        ...
        if (list.length === 0) {
          // If no history, fetch proactive greeting
        ...
        await this.fetchGreeting(); // Fallback to greeting on error
      }
    },
    async fetchGreeting() {
      try {
        ...
        if (res?.data?.status) {
          ...
          // Auto-play greeting
          setTimeout(() => {
            this.playAiMessage(greetingMsg);
          }, 800);
        }
      } catch (e) {
        this.messages = [
      ...
      try {
        ...
        if (!sessionId) {
          this.messages = [
            {
              role: "teacher",
              text: "Bạn có thể bắt đầu nhắn tin với giáo viên của bài học này.",
            },
          ];
          return;
        }
        ...
        this.messages = [
          {
            role: "teacher",
            text: "Chưa tải được lịch sử chat giáo viên, bạn thử lại nhé.",
          },
        ];
      }
    },
    async sendMessage(inputType = "text") {
      ...
      if (!token) {
        this.messages.push({
          role: "ai",
          text: "Bạn cần đăng nhập để dùng chatbox nhé.",
        });
      ...
      try {
        if (this.selectedChat.type === "teacher") {
          ...
          if (newestMessage && newestMessage.role === "user") {
            ...
            newestMessage.time = this.formatMessageTime(
              data.time || data.created_at || new Date(),
            );
          }
          const target = this.isTeacherMode
            ? this.teacherChats.find(
                (c) => c.sessionId === this.selectedChat.sessionId,
              )
            : this.teacherChats.find(
                (c) => c.teacherId === this.selectedChat.teacherId,
              );
          if (!target) {
            this.teacherChats.unshift({
              ...this.selectedChat,
              id: this.isTeacherMode
                ? `session-${this.selectedChat.sessionId}`
                : `teacher-${this.selectedChat.teacherId}`,
              isTemp: false,
            });
          }
        ...
        this.messages.push({
          ...
          text: this.parseApiErrorMessage(
            err,
            "Hiện chưa gửi được tin nhắn, bạn thử lại sau nhé.",
          ),
        });
      } finally {
        this.isTyping = false;
